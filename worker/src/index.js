...
function selectModel(userId = null, forceGemini3 = false) {
  if (forceGemini3 || FEATURE_FLAGS.useGemini3) {
    if (Date.now() > new Date('2026-03-01').getTime()) {
      return FEATURE_FLAGS.useGemini3Flash ? MODEL_GEMINI_3_FLASH : MODEL_GEMINI_3;
    }
  }
...
async function* streamGemini3Response(prompt, apiKey) {
  const response = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-3-pro:streamGenerateContent', {
    method: 'POST',
    headers: {
...
function checkModelCapabilities(model) {
  const capabilities = {
    [MODEL_GEMINI_3]: { streaming: true, multimodal: true, reasoning: true, contextWindow: 128000 },
    [MODEL_GEMINI_3_FLASH]: { streaming: true, multimodal: true, reasoning: false, contextWindow: 128000 },
    [MODEL_GEMINI_3_NANO]: { streaming: false, multimodal: false, reasoning: false, contextWindow: 32000 },
    [MODEL_FORTUNE]: { streaming: false, multimodal: false, reasoning: false, contextWindow: 8000 }
  };
...
async function generateCouncilFortune(text, apiKey) {
  ...
  for (const agent of AGENTS) {
    ...
    if (conversationHistory.length > 0) {
      messages.push({
        role: 'assistant',
        content: 'Previous council members have said: ' + conversationHistory.map(h => `${h.name}: "${h.response}"`).join(' | ')
      });
      messages.push({
        role: 'user',
    ...
    try {
      ...
      if (url.pathname === '/api/fortune/text') {
        ...
        if (selectedModel === MODEL_GEMINI_3 && FEATURE_FLAGS.useGemini3) {
          ...
          const fortune = await generateEnhancedFortune(gemini3Prompt, apiKey, capabilities);
        ...
        const stream = new ReadableStream({
          async start(controller) {
            try {
              const gemini3Prompt = buildGemini3Prompt(
                FORTUNE_PROMPT,
                { query: body.text, streaming: true }
              );
              
              for await (const chunk of streamGemini3Response(gemini3Prompt, apiKey)) {
        ...
        return new Response(JSON.stringify({
          available: [
            ...
            { id: MODEL_GEMINI_3, name: 'Gemini 3 Pro', active: FEATURE_FLAGS.useGemini3 },
            { id: MODEL_GEMINI_3_FLASH, name: 'Gemini 3 Flash', active: FEATURE_FLAGS.useGemini3Flash },
        ...
        if (body.useGemini3 !== undefined) FEATURE_FLAGS.useGemini3 = body.useGemini3;
        if (body.useGemini3Flash !== undefined) FEATURE_FLAGS.useGemini3Flash = body.useGemini3Flash;
        if (body.enableStreaming !== undefined) FEATURE_FLAGS.enableStreaming = body.enableStreaming;
